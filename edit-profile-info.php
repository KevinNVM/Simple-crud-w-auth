...
// update table
if (isset($_POST['sent'])) {
	// change $post password and $post pfp
	$_POST['user_password'] = $data['user_password'];
	$_POST['user_password2'] = $data['user_password'];
...
<div class="offcanvas offcanvas-end bg-dark" data-bs-scroll="true" tabindex="-1" id="offcanvasWithBothOptions" aria-labelledby="offcanvasWithBothOptionsLabel">
  ...
  <tbody>
  	<form method="post" enctype="multipart/form-data">
  		...
  		<tr class="edit-info d-<?= $display_i ?>">
  			<td>Username</td>
  			<td>
  				<input type="text" name="user_username" value="<?= $data['user_username'] ?>" class="form-control form-control-sm" maxlength="50" required>
  			</td>
  		</tr>
  		<tr class="edit-info d-<?= $display_i ?>">
  			<td>E-mail</td>
  			<td>
  				<input type="email" name="user_email" class="form-control form-control-sm" value="<?= $data['user_email'] ?>" required>
  			</td>
  		</tr>
  		<tr class="edit-info d-<?= $display_i ?>">
  			<td>Phone Number</td>
  			<td>
  				<input type="text" name="user_telp" class="form-control form-control-sm" value="<?= $data['user_telp'] ?>" maxlength="14" required>
  			</td>
  		</tr>
